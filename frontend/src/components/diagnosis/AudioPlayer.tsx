...
export function AudioPlayer({ textToSpeak }: AudioPlayerProps) {
  ...
  return (
    <button
      ...
      className={`w-full flex items-center justify-between p-3.5 rounded-2xl transition-all shadow-md active:scale-[0.99] ${
        isSpeaking
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-start">
          ...
          <p className="text-[11px] text-white/80">
            {isSpeaking ? 'آواز بند کرنے کے لیے کلک کریں' : 'تشخیص اور علاج اردو میں سنیں'}
          </p>
        </div>
      </div>

      <div className="flex items-center gap-1 text-xs font-semibold bg-white/20 px-3 py-1 rounded-xl">
